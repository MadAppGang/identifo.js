...
describe('UrlBuilder: ', () => {
  ...
  test('should return correct url (all params is defined)', () => {
    ...
    expect(urlBuilder.createLogoutUrl())
      .toBe(`${config.url}/web/logout?${baseParams}&callbackUrl=${config.postLogoutRedirectUri}`);
    expect(urlBuilder.createRenewSessionUrl())
      .toBe(`${config.url}/web/token/renew?${baseParams}&redirectUri=${config.redirectUri}`);
  });
  ...
  test('should return correct url (only app id & authUrl are defined)', () => {
    ...
    expect(urls.createLogoutUrl())
      .toBe(`${urlConfig.url}/web/logout?${baseParams}&callbackUrl=`);
    expect(urls.createRenewSessionUrl())
      .toBe(`${urlConfig.url}/web/token/renew?${baseParams}&redirectUri=${window.location.href}`);
  });
});
